...
│   └── AJAX API 요청 재현 시도 (응답 검증 문제로 실패)
│
└── 🧰 기타 유틸리티
...
1. `practice2.py`
    - 뉴스 목록 다중 페이지 크롤링 및 데이터 저장 (정적 HTML)
    - 기술 스택
        - requests
        - BeautifulSoup
        - 재시도 로직
...
1. `practice4.py`
    - 뉴스 동적 스크롤 페이지 AJAX 역공학 시도 (실패 기록)
    - 기술 스택
        - requests
        - AJAX Request 분석
        - Query Parameters 분석
...
- AJAX 요청이 성공하기 위한 요청 조건
    ...
    - 해당 뉴스의 경우 단순 requests GET 요청만으로는 빈 응답이 돌아옴
- AJAX 역공학 방식 사용 기준
    | 구분            | 설명                                                                                                                      |
    | ------------- | ----------------------------------------------------------------------------------------------------------------------- |
...
- 웹사이트를 자동으로 열고, 버튼 클릭, 입력, 데이터 추출 같은 일을 해주는 자동화 도구

1. Selenium
- Selenium 특징 (장점)
    - 오래되고 유명함
        - 웹 자동화에서 가장 널리 쓰이는 도구
    - 대부분의 브라우저 지원
        - Chrome, Firefox, Safari, Edge 등 거의 다 됨
    - 여러 언어 지원
...
- Selenium 단점
    - 속도가 상대적으로 느림
    - 브라우저 제어 방식이 예전 WebDriver 프로토콜이라 조금 복잡할 수 있음
    - 최신 웹 구조(React, Vue 등)에서 동기 문제 때문에 기다림 처리가 번거로울 때 있음

1. Playwright
- Playwright 특징 (장점)
    - Microsoft에서 만든 최신 자동화 도구
    - Selenium보다 빠르고 안정적
    - 모든 브라우저를 하나의 엔진으로 안전하게 제어
        - Chrome / Firefox / WebKit(=Safari 기반)까지 지원
    - 자동 대기(Auto-wait) 기능
        - 요소가 준비될 때까지 알아서 기다림 → 코드가 단순해짐
    - 웹 테스트 용도로 만들어져서 동시 실행(병렬)이 매우 쉬움
- Playwright 단점
    - Selenium만큼 오래되지 않아서 커뮤니티는 상대적으로 작음
    - 공식적으로 Python, JS, Java, .NET만 지원 (대부분 충분하지만 Selenium보단 적음)
    - 회사에서 Selenium만 쓰는 경우도 아직 많음

- 정리 표
    | 비교 항목         | Selenium         | Playwright   |
    | ------------- | ---------------- | ------------ |
    | **속도**        | 느림               | 빠름           |
    | **안정성**       | 상황에 따라 불안정할 수 있음 | 매우 안정적       |
    | ** 자동 대기 처리** | 수동 처리 필요         | 자동 처리(편함)    |
    | **지원 언어**     | 매우 많음            | 주요 언어만       |
    | **역사 & 커뮤니티** | 오래됨, 커뮤니티 큼      | 비교적 신상       |
    | **병렬 실행**     | 복잡               | 기본 지원, 매우 쉬움 |
    | **초보자 난이도**   | 약간 어려움           | 더 쉬움         |
...
### 큐(Queue)
- 먼저 들어간 데이터가 먼저 나오는(FIFO, First In First Out) 형태의 자료구조
- 현실 세계의 “대기 줄”과 동일한 개념
- 파이썬에서는 queue.Queue(), collections.deque 등으로 구현
- push / put = 뒤에 추가
- pop / get = 앞에서 꺼냄

#### 큐를 언제, 왜 사용하는가?
1. 탐색형(Discovery) 크롤링
    - ex)페이지 안에서 새로운 URL이 계속 발견되고, 그 URL을 다시 방문해야 하는 구조
        1. start_url 방문
        1. 내부 링크 추출
        1. 새 링크를 큐에 집어넣음
        1. 큐에서 다음 URL을 꺼내 방문
        ...
        1. 새 링크를 다시 큐에 넣음
        1. 반복…
    - 이 구조는 BFS(너비 우선 탐색) 알고리즘과 동일하며, BFS는 반드시 큐를 사용
        - BFS란?
            - BFS(Breadth-First Search, 너비 우선 탐색)는 가장 가까운 단계부터 차례대로 탐색하는 알고리즘
        - BFS 탐색 예시
            ```
            A
            ├─ B
            ...
            └─ G
            위와 같은 구조에서
            A → B → C → D → E → F → G 순서로 탐색
            ```
1. 여러 스레드 또는 여러 프로세스가 URL을 동시에 가져가 처리하는 경우
    - 스레드/프로세스들이 동시에 다음 URL을 가져가기 때문에 thread-safe한 자료구조(= 큐)가 필요
        - Thread-safe란?
            - 여러 스레드가 동시에 접근해도 데이터가 손상되지 않는 것
        - 리스트가 thread-safe가 아닌 이유
            - 리스트는 동시 수정을 고려하도록 설계되지 않음
        - queue.Queue()는 thread-safe인 이유
            - put() / get() 수행 시 내부적으로 Lock 사용
            - 여러 스레드가 동시에 접근해도 충돌 없음
            - 본래 멀티스레드 작업 대기열용 구조로 제작됨
1. 작업 대기열(Work Queue)을 관리
    - 큐는 작업이 들어오고 나가는 흐름을 관리하도록 만들어진 자료구조
    - 단순 저장용 리스트가 아니라, 작업 처리 흐름(workflow) 에 맞게 설계된 구조
1. 큐 vs 리스트
    | 항목                         | 큐(Queue)   | 리스트(List)          |
    | -------------------------- | ---------- | ------------------ |
    | 역할                         | 작업 처리용 구조  | 데이터 저장용 구조         |
    | 구조                         | FIFO       | 순서 있지만 FIFO 보장 안 됨 |
    | `.put()` / `.get()` 시간 복잡도 | O(1)   | 앞에서 pop 시 O(n) |
    | thread-safe                | 안전함      | 안전하지 않음          |
    | 사용 목적                      | URL 작업 대기열 | 단순 데이터 보관          |
    | 병렬 처리 시 안전성                | 높음         | 낮음                 |
    - 왜 리스트의 pop(0)은 O(n)인가?
        - 리스트는 내부적으로 배열임
        - 맨 앞 요소를 pop하면 뒤 요소들을 전부 한 칸씩 당겨야 하므로 O(n)
    - 왜 큐는 O(n)인가?
        - deque는 양쪽 끝에서 넣고 빼는 구조가 리스트와 다르게 설계되어 있어 append, popleft 모두 O(1)

#### 크롤링에서 큐 기반 구조란?
- 큐 기반 구조란, 크롤링 작업 전체를 Queue + Visited Set 조합으로 운영하는 패턴을 말함
    - 구조
        ```
        ┌─────────┐     ┌────────────┐
        │URL queue│ --> │  크롤러     │ --> 새 URL → 다시 큐에 넣기
        └─────────┘     └────────────┘
                            │
                            ↓
                    ┌────────────┐
    ...
    - 이 구조를 사용하면
        - 이미 방문한 URL은 건너뛰고
        - 아직 방문하지 않은 URL만 큐에서 꺼내며
        - 새로운 URL이 계속해서 발견되는 구조를 제대로 처리
    - 탐색형(Discovery) 크롤링 → BFS 기반 → 큐 필요
        - 방문할 URL이 계속해서 새로 생성됨 → 작업 대기열 필요
        - FIFO 구조 필요
        - 중복 제거 필요 → visited set + queue 조합
        - 멀티스레드로 확장 가능성 → deque 또는 queue.Queue 사용

#### 큐와 비동기/멀티스레드/멀티프로세싱/분산 처리의 관계
1. 비동기(asyncio)
    - 수많은 네트워크 요청을 빠르게 보내고 싶을 때 사용
    - URL 대기열 관리에는 큐를 사용
    - asyncio.Queue()를 사용하면 비동기 작업 대기열 관리 가능
1. 멀티스레드(Threading)
    - I/O-bound 크롤링에 적합
    - 여러 스레드가 동시에 queue.get() 해서 작업을 병렬 처리
    - 큐가 thread-safe이므로 충돌 없음
1. 멀티프로세싱(Multiprocessing)
    - CPU-bound 작업이나 대규모 병렬 처리에 사용
    - 프로세스 간 Queue는 OS 레벨에서 관리됨
1. 분산 처리(컴퓨터 여러 대)
    - 이때는 파이썬 내부 queue로 처리 불가
    - 꼭 Redis/RabbitMQ/Kafka 같은 “분산 메시지 큐” 필요

#### 최종 요약
- 큐는 “작업 흐름 관리”를 위한 구조이며 리스트와 목적 자체가 다름
- 탐색형 크롤링에서는 BFS 구조이므로 큐가 정석
- 리스트는 thread-safe하지 않으며 pop(0) 성능도 나쁨
- 큐는 thread-safe하고 작업 처리에 최적화됨
- 크롤링 규모가 커질수록 → 비동기, 멀티스레드, 멀티프로세싱, 분산 처리로 확장
- 분산 처리 시 Redis/RabbitMQ 같은 외부 메시지 큐 필